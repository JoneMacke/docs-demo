...
| 配置项 | 值 |
|--------|-----|
| API 地址 | `https://api.katioai.com`（注意：不要加 `/v1`） |
| API 协议 | `anthropic-messages` |
| API 密钥 | 你在 [枫叶AI 控制台](https://api.katioai.com/console) 获取的密钥 |
...
::: warning ⚠️ 配置注意事项
- ✅ `baseUrl` 填写 `https://api.katioai.com`，**不要包含 `/v1` 后缀**
- ✅ `api` 协议必须选择 `anthropic-messages`
- ✅ `apiKey` 替换为你在 [枫叶AI 控制台](https://api.katioai.com/console) 获取的实际密钥
...
{
  "provider": "openai",
  "base_url": "https://api.katioai.com",
  "api": "anthropic-messages",
...
| `base_url` | API 接入地址：`https://api.katioai.com` |
| `api` | 协议类型：`openai-completions` 或 `anthropic-messages` |
| `api_key` | 你在 [枫叶AI 控制台](https://api.katioai.com/console) 获取的密钥 |
...
::: details 连接失败怎么办？
1. 检查 `baseUrl` 是否正确填写为 `https://api.katioai.com`（不含 `/v1`）
